...
// TestReadMountInfoExceeds1MiB: a mountinfo file larger than 1 MiB is read and parsed in full.
func TestReadMountInfoExceeds1MiB(t *testing.T) {
	...
	// util.ReadFileNoStat caps at 1 MiB and would truncate this file.
...
// TestParseMountInfoSkipsMalformedLine: a torn line is skipped, not fatal to the parse.
func TestParseMountInfoSkipsMalformedLine(t *testing.T) {
	valid1 := "16 21 0:16 / /sys rw,nosuid,nodev,noexec,relatime shared:7 - sysfs sysfs rw"
	valid2 := "36 35 98:0 /mnt /mnt rw,noatime shared:1 - ext3 /dev/root rw,errors=continue"
	torn := "5800 28 0:1294"

	info := []byte(valid1 + "\n" + torn + "\n" + valid2 + "\n")

	mounts, err := parseMountInfo(info)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(mounts) != 2 {
		t.Fatalf("got %d mounts, want 2 (the torn line must be skipped, not fatal)", len(mounts))
	}
}
